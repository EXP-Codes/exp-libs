...
public class _Minute extends __TimeUnit {
	 ...
	 * @return
	 */
	private boolean inRange(int minute) {
	...
	protected boolean _checkSubExpression(String subExpression) {
		...
		// 子表达式为: [x] 或 [ a,b,c,...,n]
			...
			isOk = true;
			for(String minute : sequence) {
				isOk &= inRange(NumUtils.toInt(minute, -1));
			}
	...
	protected void _trigger(Cron cron, String subExpression) {
		...
		// [分]字段的子表达式为: [*] 时, 比[分]大的时间单位不再有参考意义
		if(EVERY.equals(subExpression)) {
		...
		// 比[分]小的时间单位若为 [*] 则自动变成 [最小值]
		if(cron.Second().isEvery()) {
			cron.Second()._setSubExpression(_Second.MIN);
		}
	}
